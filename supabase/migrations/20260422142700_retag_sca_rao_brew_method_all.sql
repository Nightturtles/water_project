-- =============================================================================
-- Cafelytic — editorial re-tag sca + rao
--
...
-- was an oversight in the seed-presets migration — they should surface in
-- both the Filter and Espresso filter views of the recipe browser.
--
...
-- CHECK constraint allowing 'all' on library rows landed in the
-- brew_method-check migration. This migration is pure data: one UPDATE
-- statement affecting two slugs, no schema change.
--
...
-- where the seed-presets migration wasn't applied) or somehow duplicated —
-- surfaces catalog drift instead of silently no-op'ing.
-- =============================================================================
