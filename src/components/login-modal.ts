...
function ensureModal(): HTMLElement {
  ...
  modalEl.innerHTML = [
    ...
    "  </button>",
    '  <button type="button" class="login-apple-btn login-modal-apple">',
...
function bindModalHandlers(root: HTMLElement): void {
  ...
  const appleBtn = $(root, ".login-modal-apple") as HTMLButtonElement;
  ...
  appleBtn.addEventListener("click", async function () {
    ...
    clearMessages();
    try {
      const res = await (window as any).signInWithApple();
      if (res && res.error) {
        showError(res.error.message);
