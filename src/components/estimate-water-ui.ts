...
// it; applyAuthGate (ui-shared.ts) visually locks the open button for
...
//
// Phase A: converted from estimate-water-ui.js. Loaded via legacy-globals.ts
...
// (script.js, the recipe.html inline DOMContentLoaded block) reach
...
// Shape of a successful estimate-water Edge Function response. Non-ok
// responses are turned into thrown errors inside invokeEstimateFunction, so
// callers only ever see this success shape.
...
function cacheKey(zip: string, provider: string): string {
  return CACHE_KEY_PREFIX + String(zip) + ":" + providerSlug(provider);
}

function readCache(zip: string, provider: string): EstimateCacheEntry | null {
  try {
    ...
    return parsed as EstimateCacheEntry;
  } catch (_) {
    return null;
  }
}

function writeCache(zip: string, provider: string, payload: EstimateCacheEntry): void {
  try {
...
function reportToSentry(error: unknown, extra?: Record<string, unknown>): void {
  try {
...
function applyProfileToInputs(profile: Record<string, unknown>): void {
  ...
  const customBtn = document.querySelector<HTMLElement>('#source-presets [data-preset="custom"]');
  if (customBtn) customBtn.click();

  if (typeof ION_FIELDS === "undefined" || !Array.isArray(ION_FIELDS)) return;
  ION_FIELDS.forEach(function (ion) {
    const el = document.getElementById("src-" + ion) as HTMLInputElement | null;
    if (!el) return;
    const value = Number(profile[ion]);
    el.value = String(Number.isFinite(value) ? Math.max(0, value) : 0);
    el.dispatchEvent(new Event("input", { bubbles: true }));
  });
}

async function invokeEstimateFunction(
  body: Record<string, unknown>,
  timeoutMs: number,
  options?: InvokeOptions,
): Promise<EstimateSuccess> {
  options = options || {};
  ...
  try {
    ...
    if (error) {
      ...
      const ctx = (
        error as { context?: { json?: () => Promise<{ error?: string; message?: string }> } }
      ).context;
...
function callEstimate(zip: string, provider: string): Promise<EstimateSuccess> {
...
function initEstimateWaterUI(): void {
  const card = document.getElementById("estimate-water-card");
  if (!card) return; // page doesn't host the feature
  const openBtn = document.getElementById("estimate-open-btn") as HTMLButtonElement | null;
  const form = document.getElementById("estimate-form");
  const zipInput = document.getElementById("estimate-zip") as HTMLInputElement | null;
  const providerInput = document.getElementById("estimate-provider") as HTMLInputElement | null;
  const submitBtn = document.getElementById("estimate-submit-btn") as HTMLButtonElement | null;
  const cancelBtn = document.getElementById("estimate-cancel-btn") as HTMLButtonElement | null;
  ...
  function setStatus(message: string, severity?: string): void {
    if (!statusEl) return;
  ...
  function showLastResult(result: EstimateCacheEntry, zip: string, provider: string): void {
  ...
  async function submitEstimate(opts?: { bypassCache?: boolean }): Promise<void> {
    opts = opts || {};
    ...
    try {
      ...
      const payload: EstimateCacheEntry = {
      ...
      const code =
        err && (err as { code?: string }).code ? (err as { code: string }).code : "model_error";
      ...
      const errMessage = err && (err as { message?: string }).message;
      const useServerMessage = code === "daily_limit" && errMessage;
      setStatus(useServerMessage ? (errMessage as string) : friendlyError(code), "error");
  ...
  // (see ui-shared.ts). The server is the real boundary — even if a user
