...
| Calc | `metrics.js`, `src/lib/constants.ts` (mineral DB, presets, conversion constants + the shared ion/mineral types) |
...
- **Phase 3** — Vite + TypeScript strict bundling. **Infrastructure landed**: Vitest + Playwright + ESLint/`tsc --noEmit`, the Vite scaffold (PR a), the dev-server + TS test migration (PR b), the Pages deploy cutover (PR c), the storage/sync move to `src/lib/*.ts` plus the `legacy-globals.ts` bridge (PR d), and the first UI slice (ui-shared.js + login-modal.js → `src/components/*.ts`, PR e). `supabase-client` and `sentry-init` have likewise landed as `src/lib/*.ts`. The classic UI/data migration is done, and the constants layer has since moved from the classic root script to `src/lib/constants.ts` (typed module; `legacy-globals.ts` also `Object.assign`s it onto window for the still-classic metrics.js and the inline page scripts). `metrics.js` is the last remaining migration target. `theme-init.js` is the one intentional permanent classic — a render-blocking `<head>` primer (loaded via `partials/head-bottom.html`) that a deferred ES module would break (it would run after first paint and reintroduce FOUC plus the native inter-page flash; `smoke-index.spec.ts` guards against converting it). It is `@ts-check`'d + linted alongside `metrics.js`, so it is an intentional classic, not an outstanding migration.
