...
**Codified note**: `smoke-sync.spec.ts` covers a *sequenced* variant of this step (A edits, await B receiving via Realtime, then B edits). The *strict-concurrent* variant above stays as a manual UX walk because of a known read-replica race in `scheduleRealtimePull` ([sync.js:824](../sync.js)): when A's broadcast-triggered pull lands while A's own push is mid-flight, the `SELECT` in `pullFromCloud` can hit a lagging Supabase read replica and briefly overwrite A's localStorage with stale cloud state. PR #86's dirty-tracking makes this non-destructive — cloud state is correct (A's push did commit), and the next sync cycle detects the localStorage/cloud mismatch and recovers — but A's UI may briefly flash the pre-edit value before the next pull resolves. If you ever see *cloud state* diverge (A's edit missing from `target_profiles` after the dust settles) or *both reloads* show stale data, that's a real regression, not the known race.
